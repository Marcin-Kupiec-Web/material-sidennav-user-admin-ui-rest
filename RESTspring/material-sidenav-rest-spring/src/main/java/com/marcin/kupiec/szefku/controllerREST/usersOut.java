...
@GetMapping(path = "/getPrivileges")
    public UserObjectRest getPrivileges() {
    	UserObjectRest urest=new UserObjectRest();
    	urest.setPlm(pr.findAll());
        return urest; 
    }
...
@PutMapping("/updateRoles")
	...
	Role rolaSave= rr.findById(rolaDetails.getId()).orElseThrow(() -> new ResourceNotFoundException("User not found for this id :: " + rolaDetails.getId()));
	...
	for(Privilege pri:rolaDetails.getPrivileges()) {
		Privilege prSave=pr.findById(pri.getId()).orElseThrow(() -> new ResourceNotFoundException("Role not found for this id :: " + pri.getId()));;
	   prl.add(prSave);
	}
...
@PutMapping("/updatePrivileges")
	...
	Privilege privSave= pr.findById(privilegeDetails.getId()).orElseThrow(() -> new ResourceNotFoundException("User not found for this id :: " + privilegeDetails.getId()));
...
@DeleteMapping(path ={"deleteUsers/{id}"})
		public User removeUser(@PathVariable("id") int id) throws ResourceNotFoundException {
			User user = usr.findById(id).orElseThrow(() -> new ResourceNotFoundException("Role not found for this id :: " + id));
				if(user != null){
						usr.delete(user);
					}
				return user;
...
@DeleteMapping(path ={"deletePrivileges/{id}"})
		public Privilege removePrivilege(@PathVariable("id") int id) throws ResourceNotFoundException {
			Privilege priv = pr.findById(id).orElseThrow(() -> new ResourceNotFoundException("Nie znaleziono id :: " + id));
				if(priv != null){
						pr.delete(priv);
					}
				return priv;
